...
import { createCardService, findCardByAccountNumberService, findCardsServices, updateAvailableMoneyService } from "../services/bank";
...
export const createCard = async (event: APIGatewayProxyEvent) => {
  ...
  try {
    const res = await createCardService( accountNumber, availableMoney, typeCard)
